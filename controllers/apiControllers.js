...
module.exports.testPinterestApi = async function testPinterestApi(req, res) {
  const { url, token, params } = req.body; // Get URL, token, and params from the request body
  ...
  try {
      ...
      while (hasMore) {
          const response = await retryWithDelay(async () => {
              return await axios.get(fetchUrl.toString(), {
                  headers: {
          ...
          const itemsChunk = response.data.items;
          allItems = allItems.concat(itemsChunk);
          totalFetched += itemsChunk.length;

          console.log(`Fetched ${itemsChunk.length} items. Total so far: ${totalFetched}`);

          // Check for pagination
          currentBookmark = response.data.bookmark;
          hasMore = !!currentBookmark;

          // If there is a bookmark, append it to the URL for the next request
          if (hasMore) {
              fetchUrl.searchParams.set('bookmark', currentBookmark);
          }
      ...
      console.log(`Finished fetching data from ${url}. Total items fetched: ${totalFetched}`);
